...
## 🛡️ How the Built-In Region Bypass Works (`mitmdump`)
...
3. **Payload Manipulation:** The internal proxy addon intercepts and modifies critical Google Cloud Code API responses on the fly:
   ...
   - **`fetchUserInfo` / `fetchAdminControls`**: Overrides your account profile `"regionCode"` to `"US"`.
...
- **Update Immunity:** Because `gem` intercepts traffic dynamically rather than patching files, updating official Antigravity (`agy update`) will **never** break or overwrite your region bypass.
...
Commands & Options:
  ...
  -d, --delete SUFFIX      Delete session agy-<suffix> (or 'agy' if no suffix)
